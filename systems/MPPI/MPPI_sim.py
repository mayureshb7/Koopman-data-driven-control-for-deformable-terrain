...
from verti_bench.systems.MPPI.MPPI import MPPIPlanner
...
class MPPISim:
    def __init__(self, config, logger=None):
        ...
        self.config = config
        self.logger = logger
        self._terrain_ref_height = None
        # self._terrain_ref_height = None
        self._prev_log_pos = None
    ...
    def _sample_deformable_terrain(self, terrain, xs, ys):
        samples = []
        for x in xs:
            for y in ys:
                p = chrono.ChVector3d(x, y, 0.0)
                h = terrain.GetHeight(p)
                n = terrain.GetNormal(p)
                mu = terrain.GetCoefficientFriction(p)
                samples.append({
                    "x": float(x),
                    "y": float(y),
                    "height": float(h),
    ...
    def _log_tire_forces(self, terrain):
        tire_forces = []

        vehicle = self.vehicle_manager.vehicle.GetVehicle()
        ...
        for axle in range(num_axles):
            for side in [veh.LEFT, veh.RIGHT]:
                ...
                tire_forces.append({
                    "axle": axle,
                    "side": "left" if side == veh.LEFT else "right",
                    "fx": float(tf.force.x),
                    "fy": float(tf.force.y),
                    "fz": float(tf.force.z),
                    "px": float(tf.point.x),
                    "py": float(tf.point.y),
                    "pz": float(tf.point.z),
                    "mx": float(tf.moment.x),
                    "my": float(tf.moment.y),
                    "mz": float(tf.moment.z),
                })

        return tire_forces

    def _log_step(self, time, vehicle_pos, roll, pitch, vehicle_heading,
              m_driver_inputs, scm_terrain=None, terrain_samples=None,
              tire_forces=None):
        if self.logger is None:
            return

        if self._prev_log_pos is None:
            speed = 0.0
        else:
            dx = vehicle_pos.x - self._prev_log_pos.x
            dy = vehicle_pos.y - self._prev_log_pos.y
            dz = vehicle_pos.z - self._prev_log_pos.z
            speed = np.sqrt(dx * dx + dy * dy + dz * dz) / self.step_size

        self._prev_log_pos = chrono.ChVector3d(vehicle_pos.x, vehicle_pos.y, vehicle_pos.z)

        terrain_height = None
        terrain_deformation = None
        terrain_type = None

        if scm_terrain is not None:
            p = chrono.ChVector3d(vehicle_pos.x, vehicle_pos.y, vehicle_pos.z)
            terrain_height = scm_terrain.GetHeight(p)
            if self._terrain_ref_height is None:
                self._terrain_ref_height = terrain_height
            terrain_deformation = terrain_height - self._terrain_ref_height
            terrain_type = "deformable"

        self.logger.log({
            "time": float(time),
            "x": float(vehicle_pos.x),
            "y": float(vehicle_pos.y),
            "z": float(vehicle_pos.z),
            "roll": float(roll),
            "pitch": float(pitch),
            "yaw": float(vehicle_heading),
            "speed": float(speed),
            "steering": float(m_driver_inputs.m_steering),
            "throttle": float(m_driver_inputs.m_throttle),
            "braking": float(m_driver_inputs.m_braking),
            "terrain_height": float(terrain_height) if terrain_height is not None else None,
            "terrain_deformation": float(terrain_deformation) if terrain_deformation is not None else None,
            "terrain_type": terrain_type,
            "terrain_samples": terrain_samples,
            "tire_forces": tire_forces,
            "goal_x": float(self.vehicle_manager.goal.x),
            "goal_y": float(self.vehicle_manager.goal.y),
            "goal_z": float(self.vehicle_manager.goal.z),
        })
    ...
    def _initialize_system(self):
        ...
        if self.system_type.lower() == 'mppi':
            self.planner = MPPIPlanner(self.terrain_manager)
        else:
    ...
    def run(self):
        ...
        for terrain in self.terrains:
            if isinstance(terrain, veh.SCMTerrain):
                scm_terrain = terrain
                break
        ...
        while True:
            ...
            # Sample deformable terrain around the vehicle
            terrain_samples = None
            if scm_terrain is not None:
                xs = np.linspace(vehicle_pos.x - 2.0, vehicle_pos.x + 2.0, 5)
                ys = np.linspace(vehicle_pos.y - 2.0, vehicle_pos.y + 2.0, 5)
                terrain_samples = self._sample_deformable_terrain(scm_terrain, xs, ys)
            ...
            if self.use_gui:
                ...
                if time - self.last_replan_time >= self.replan_interval:
                    ...
                    if scm_terrain is not None:
                        tire_forces = self._log_tire_forces(scm_terrain)
                    else:
                        tire_forces = self._log_tire_forces(terrain)
                ...
                tire_forces = None
                if self.terrains:
                    terrain_for_forces = None
                    for terrain in self.terrains:
                        if isinstance(terrain, veh.SCMTerrain):
                            terrain_for_forces = terrain
                            break
                    if terrain_for_forces is None:
                        terrain_for_forces = self.terrains[0]

                    tire_forces = self._log_tire_forces(terrain_for_forces)

            # Log once, after roll/pitch/yaw and controls exist
            self._log_step(
                time, vehicle_pos, roll, pitch, vehicle_heading,
                self.driver_inputs, scm_terrain, terrain_samples, tire_forces
            )
            ...
            if self.last_position:
                ...
                if self.stuck_counter >= self.stuck_time:
                    ...
                    if self.logger is not None:
                        self.logger.save(f"vehicle_log_world{self.world_id}.csv")
